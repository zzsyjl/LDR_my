...
class IncreMCRGANloss(nn.Module):

    def __init__(self, gam1=1., gam2=1., gam3=1., gam4=1., eps=0.5, numclasses=1000, mode=1, rho=None):
        super(IncreMCRGANloss, self).__init__()
    ...
    def compute_discrimn_loss(self, Z):
        ...
        d, n = Z.shape # d=128, n=4096
        I = torch.eye(d).to(Z.device)
        scalar = d / (n * self.eps) # self.eps=0.5 denote as the eps^2 term
    ...
    def compute_compress_loss(self, Z, Pi):
        ...
        for j in range(Pi.shape[1]):
            Z_ = Z[:, Pi[:, j] == 1] # mask and filter
            trPi = Pi[:, j].sum() + 1e-8 # 400
            scalar = d / (trPi * self.eps) # 0.64
            log_det = 1. if Pi[:, j].sum() == 0 else self.logdet(I + scalar * Z_ @ Z_.T) # 55.61
            compress_loss.append(log_det)
            scalars.append(trPi / (2 * n)) # 0.0508
